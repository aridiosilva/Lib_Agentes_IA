<!--
  Como usar: Cole em uma conversa nova do Claude Code, aberta na pasta do projeto em que o agente vai trabalhar.
...
1. Escreva o conteúdo entre <agente> e </agente> em um arquivo `CLAUDE.md` na raiz do projeto.
2. Se já existir um `CLAUDE.md`, acrescente as seções ao que está lá em vez de sobrescrever o arquivo.
3. A partir daí, siga essas instruções em todas as respostas deste projeto.
4. Use apenas as ferramentas listadas na seção Tools, respeitando a permissão declarada em cada uma. Se faltar acesso, diga o que falta em vez de improvisar.
